...
logging_cleanup_old() {
    ...
    if [ -d "$LOG_DIR" ]; then
        # Count log files
        local log_count
        log_count=$(find "$LOG_DIR" -maxdepth 1 -name "install-*.log" 2>/dev/null | wc -l)

        if [ "$log_count" -gt "$keep_count" ]; then
            # Remove oldest logs, keep only $keep_count most recent
            find "$LOG_DIR" -maxdepth 1 -name "install-*.log" -print0 | xargs -0 ls -1t | tail -n +$((keep_count + 1)) | xargs rm -f
