...
finalize_installation() {
    ...
    # Only check GUI components if GUI mode is enabled
    if [ "$INSTALL_GUI_ESSENTIAL" = true ]; then
        ...
        print_info "TUI-only installation - skipping GUI component checks"
        log_info "TUI-only installation - skipping GUI component checks"
    fi
